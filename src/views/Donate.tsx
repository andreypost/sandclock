...
const Donate: React.FC<Props> = ({ suggestions, setSuggestions }: Props, { boxIndex = 0, inputIndex = 0, title, type, mission }: Options) => {
  ...
  const handleEpisodeCollectionView = (e: { target: any }) => {
    ...
    document.querySelector<HTMLHeadingElement>('.suggestion .active')?.classList.remove('active')
    target.classList.add('active')
    document.querySelector('.orgs_container>.active')?.classList.remove('active')
    const container = document.querySelector<HTMLDivElement>('.orgs_container')
    if (container) container.style.height = document.querySelectorAll('.orgs_container .boxes')[target.dataset.tab]?.clientHeight + 62 + 'px'
    document.querySelectorAll<HTMLDivElement>('.orgs_container .boxes')[target.dataset.tab]?.classList.add('active')
  }
  ...
  const handlePercentLevelDonate = (e: { target: any }) => {
    let percent = 0
    e.target.closest('main').querySelectorAll('.donate_item input').forEach((item: HTMLInputElement) => percent += +item.value)
    setLevelDonate({ value: percent, submit: percent > 0 ? 'active' : '', class: percent > 100 ? 'active' : '' })
  }
    ...
    <div className={'fallback donatepage ' + opacity}>
      <header>
        ...
        <Banner>
          ...
          <h5>Donating to a charity that is close to your heart? Supporting your alma mater? Or perhaps you have an individual in mind.</h5>
          <h5>Choose below, or add your own! We will ensure they will get these funds, and we will issue you a refund equivalent to your principal if you choose to keep it.</h5>
          <form action="" id="orgsSearch">
            <input type="search" pattern="[^'\x22]+" placeholder="Start Typing..." />
          </form>
        </Banner>
      </header>
      <main>
        <div className="orgs">
          <div className="orgs_section section">
            <article className="suggestion">
              {suggestions && suggestions.length > 0 && suggestions.map((obj, index) =>
                <h3 className={(index === 0 ? 'active' : '')} key={obj.header} data-tab={index} onClick={handleEpisodeCollectionView}>{obj.header}</h3>
              )}
            </article>
            <div className="orgs_container">
              {suggestions && suggestions.length > 0 && suggestions.map((obj, index) =>
                <div className={'boxes ' + (index === 0 ? 'active' : '')} key={obj.header}>
                  <div className="individual" onClick={() => setOwnForm({ form_1: 'active', form_2: '' })}>
                    <h4>Individual <br /> / <br /> Add Your Own</h4>
                  </div>
                  {obj.orgs.map((orgs) =>
        ...
        <div className="allocation section">
          <form action="" id="allocationForm" onSubmit={handleSubmitAllocationForm}>
            <section>
              <h4>Your Allocation</h4>
              <div className="yield">
                <p data-tooltip="Yield is the profit generated by the strategy. We will refund the rest.">Yield</p>
                <div onClick={e => e.currentTarget.classList.toggle('active')} />
                <p data-tooltip="Choose this if you want to donate everything.">Whole</p>
              </div>
              <p className="total_donate">Total:<span className={levelDonate.class}>{levelDonate.value}%</span></p>
            </section>
            <div className="allocation_container">
              {suggestions && suggestions.length > 0 && suggestions.map(obj =>
                obj.orgs.map((orgs) =>
                  <div key={orgs.title} className="donate_item" data-index={inputIndex}>{orgs.title}
                    <label>
                      <input type="hidden" name={orgs.title} data-index={inputIndex++} defaultValue=""
            ...
            <button type="submit" className={levelDonate.submit} form="allocationForm">Proceed</button>
          </form>
        </div>
      </main>
      ...
      <div className={'modal ' + ownForm.form_1} onClick={e => { if (e.target === e.currentTarget) setOwnForm({ form_1: '', form_2: '' }) }}>
        <form action="" id="individualStartForm" onSubmit={handleOwnTypeForm}>
          ...
          <input type="text" name="name" id="name" value={nameOwnForm} placeholder="Name Surname" pattern="[A-Za-z]{2,}\s[A-Za-z]{2,}" onChange={e => setNameOwnForm(e.target.value)} autoComplete="off" required />
          ...
          <div className="individual_city">
            <input type="text" name="city" id="city" placeholder="City" value={cityOwnForm} onChange={e => setCityOwnForm(e.target.value)} autoComplete="off" />
            <input type="text" name="state" id="state" placeholder="State" value={stateOwnForm} onChange={e => setStateOwnForm(e.target.value)} autoComplete="off" />
            <input type="text" name="code" id="code" placeholder="Zip Code" value={codeOwnForm} onChange={e => setCodeOwnForm(e.target.value)} autoComplete="off" />
          </div>
          <button type="submit" form="individualStartForm">→</button>
        </form>
      </div>
      <div className={'modal ' + ownForm.form_2} onClick={e => { if (e.target === e.currentTarget) setOwnForm({ form_1: 'active', form_2: '' }) }}>
        <form action="" id="individualSubmitForm" onSubmit={handleOwnSubmitForm}>
          <label htmlFor="email">Email (if known)</label>
          <input type="email" name="email" id="email" value={emailOwnForm} placeholder="email@com.net" onChange={e => setEmailOwnForm(e.target.value)} pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$" autoComplete="off" />
          <label htmlFor="ein">EIN (if known/applicable)</label>
          <input type="text" name="ein" id="ein" value={einOwnForm} placeholder="XX-XXXXXXX" pattern="[0-9]{2}-[0-9]{8,}" onChange={e => setEinOwnForm(e.target.value)} autoComplete="off" />
          <label htmlFor="message">Message or Additional Information (optional)</label>
          <textarea name="message" id="message" value={messageOwnForm} onChange={e => setMessageOwnForm(e.target.value)} autoComplete="off"></textarea>
          <div>
            <input type="button" value="←" onClick={() => setOwnForm({ form_1: 'active', form_2: '' })} />
            <button type="submit" form="individualSubmitForm">Add</button>
          </div>
        </form>
      </div>
      <div className={'modal orginfo ' + orgInfoOverlay.class} onClick={e => { if (e.target === e.currentTarget) setOrgInfoOverlay({ value: { title: '', type: '', mission: '' }, class: '' }) }}>
        <div>
          ...
          <p className="goBack" onClick={() => setOrgInfoOverlay({ value: { title: '', type: '', mission: '' }, class: '' })}>← Go back</p>
        </div>
      </div>
    </div >
  )
