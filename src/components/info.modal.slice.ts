...
export const infoModalSlice = createSlice({
  ...
  reducers: {
    ...
    fieldInfoModal: (state) => {
      state.value =
        'We need you to fill in an additional field of your choice to ensure your donation will go to the right place.'
      state.style = 'active'
    },
    ...
    errorInfoModal: (state) => {
      state.value =
        'Sorry, but it is not possible to subscribe right now. Try later!'
      state.style = 'active'
    },
    exceedInfoModal: (state) => {
      state.value =
        'Your total allocation cannot exceed 100%. Please review your allocation(s)!'
      state.style = 'active'
    },
    sentInfoModal: (state) => {
